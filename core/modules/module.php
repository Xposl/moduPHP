 ...
 */
function _module_include_refer_sys($sys,$use_modules=array()){
  global $modules, $database;
  require_once(CANROOTPATH.'/core/modules/_modules_.php');
  $root_path = SYSROOTPATH.'/'.$sys;
  $modules = new Modules($sys,$root_path,$use_modules);
  if($modules->db){
    $database = $modules->db;
  }
  return $modules;
