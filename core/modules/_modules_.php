...
class Modules {
    ...
    public function __construct($root_path = '', $use_modules = array()) {
        $this->root_path = $root_path;
        $this->modules = array(
            'modules' => array(),
            'related' => array(),
        );
        $this->get_sys($root_path, $use_modules);
    }

    private function get_sys($root_path, $use_modules) {
        $this->root_path = $root_path;
        $analysis = array(
            'results' => array(),
            'info' => array(),
        ...
        if (file_exists($this->root_path)) {
            ...
            if ( file_exists($this->root_path . '/main.info')) {
                $configs = config_get_config($this->root_path . '/main.info');
                $this->name = isset($configs['name']) ? $configs['name'] : '';
                $this->sysname = isset($configs['sysname']) ? $configs['sysname'] : '';
                
                //TODO: check use which type of databas
                if(!empty($configs['db'])){
                    $this->db = array();
                    foreach( $configs['db'] as $key => $db){
                        $hostname = $db['hostname'];
                        $database = $db['database'];
                        $username = $db['username'];
                        $password = $db['password'];
                        if ( $hostname && $database && $username && $password) {
                            $this->db[$key] = new DB($hostname, $username, $password, $database);
                        }
                    }
                }
                unset($configs['db']);
            ...
            //load base file of sys
            if ( file_exists($this->root_path . '/main.base')) {
                require_once($this->root_path . '/main.base');
            }
            $this->refer_sys_modules_lists($this->root_path.'/modules');
        }
    ...
    private function refer_sys_modules_lists($path) {
        foreach (array_slice(scandir($path), 2) as $file) {
            ...
            if (is_file($module_file) && isset($module_name['extension']) && $module_name['extension'] == 'module') {
                ...
                $hook_fun_name = $module_name . "_module_info";
        ...
        try {
            ...
            if (empty($requires)) {
                ...
                foreach ($requires as $require) {
                    if (isset($results['waits'][$require]))
                        throw new Exception('Require Looped Error For :' . $current);
    ...
    public function module_invoke_functions($function_name, $args = array()) {
        foreach ($this->modules['order'] as $module_name) {
            $hook_fun_name = $module_name . "_" . $function_name;
            if (function_exists($hook_fun_name)) {
